...
//     this.#imageEl.src = this.#imageConfig.src
...
//     const finalHeight =
//       cfg.height && cfg.height > 0 ? cfg.height : naturalHeight
//     const shadowExtraX = cfg.showShadow
//       ? Math.max(cfg.shadowOffsetX ?? 0, 0) + (cfg.shadowSpread ?? 0)
//       : 0
//     const shadowExtraY = cfg.showShadow
//       ? Math.max(cfg.shadowOffsetY ?? 0, 0) + (cfg.shadowSpread ?? 0)
//       : 0
//     const xPadding = cfg.showShadow
//       ? Math.abs(Math.min(cfg.shadowOffsetX ?? 0, 0)) + (cfg.shadowSpread ?? 0)
//       : 0
//     const yPadding = cfg.showShadow
//       ? Math.abs(Math.min(cfg.shadowOffsetY ?? 0, 0)) + (cfg.shadowSpread ?? 0)
//       : 0
...
//     this.#callback(this.#cvsEl.width, this.#cvsEl.height)
//   }

//   async tick (
//     time: number
//   ): Promise<{ video?: VideoFrame; state: 'success' | 'done' }> {
//     const cfg = this.#imageConfig
...
//     //draw final image
//     const finalWidth =
//       cfg.width && cfg.width > 0 ? cfg.width : this.#imageEl.naturalWidth
//     const finalHeight =
//       cfg.height && cfg.height > 0 ? cfg.height : this.#imageEl.naturalHeight
...
//       //draw border if there's config for it
...
//   }
//   //generate an image(PNG) from the given frame ie tick(0)
//   async generateImage () {
